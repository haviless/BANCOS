...
procedure TFCancelLiq.FormActivate(Sender: TObject); //ok rlp
begin
...
procedure TFCancelLiq.MuestraDatos;
begin
...
procedure TFCancelLiq.IniciaDatos;
begin
...
{   xSQL:='Select * from CXC304 '
   ...
   DM1.cdsDetFPago.EmptyDataSet;
    ...
    begin
      xSQL:='SELECT CXC320.*, TGE103.TMONABR, TGE105.BANCOABR '+
            'FROM CXC320, TGE103, TGE105 '+
            'WHERE CIAID='''' '+
            'AND TGE103.TMONID=CXC320.TMONIDPAGO '+
            'AND TGE105.BANCOID=CXC320.BANCOID';
...
*******************************************************}
procedure TFCancelLiq.edtClieRUCExit(Sender: TObject);
var
...
procedure TFCancelLiq.dbeTCambioExit(Sender: TObject);
begin
...
procedure TFCancelLiq.z2bbtnOKClick(Sender: TObject);    //rlp ok
var
   xWhere, xSQL :string;
begin
    ValidaCab ;
...
procedure TFCancelLiq.RecalculaSaldos(xNuevo:boolean);
begin
...
procedure TFCancelLiq.z2bbtnCancelClick(Sender: TObject);  //rlp ok
begin
...
procedure TFCancelLiq.dbgPendientesEndDrag(Sender, Target: TObject; X, Y: Integer);
begin
end;

// Para Recibir los Documentos de Canje

procedure TFCancelLiq.dbgPendientesDragOver(Sender, Source: TObject; X,
...
procedure TFCancelLiq.dbgDocCanjeEndDrag(Sender, Target: TObject; X,
  Y: Integer);
begin
end;
...
procedure TFCancelLiq.dbgDocCanjeDragOver(Sender, Source: TObject; X,
  Y: Integer; State: TDragState; var Accept: Boolean);
begin
end;
...
procedure TFCancelLiq.z2bbtnCalcClick(Sender: TObject);
begin
   WinExec('C:\windows\calc.exe',1 );  // Executa Aplicación
end;

procedure TFCancelLiq.z2bbtnSumatClick(Sender: TObject);
Var
...
{      DM1.cdsQry2.DisableControls;
      ...
      begin
         xTSalLoc := xTSalLoc + DM1.dsQry2.FieldByName('LICOMTOABOLOC').AsFloat;
         xTSalExt := xTSalExt + DM1.dsQry2.FieldByName('LICOMTOABOEXT').AsFloat;
         DM1.cdsQry2.Next;
      end;

      dbgDocCanje.ColumnByName('LICOMTOABOLOC').FooterValue:=floattostrf(xTSalLoc, ffNumber, 15, 2);
      dbgDocCanje.ColumnByName('LICOMTOABOEXT').FooterValue:=floattostrf(xTSalExt, ffNumber, 15, 2);

      DM1.cdsQry2.GotoBookmark(xRegAct);
      DM1.cdsQry2.FreeBookmark(xRegAct);
...
procedure TFCancelLiq.z2bbtnSubeClick(Sender: TObject);
begin
   DM1.cdsQry2.EmptyDataSet;
   DesabilitaClient();
   pnlCabecera.Enabled:= True;
...
function TFCancelLiq.ConsisMonto:Boolean;
...
procedure TFCancelLiq.z2bbtnGrabaClick(Sender: TObject);
var
  xRegAct : TBookMark;
...
procedure TFCancelLiq.z2bbtnCancel2Click(Sender: TObject);
begin
...
procedure TFCancelLiq.z2bbtnImprimeClick(Sender: TObject);
begin
	 //JCC: 24/06/2002
...
procedure TFCancelLiq.ppDBText5Print(Sender: TObject);
begin
...
procedure TFCancelLiq.ppDBCalc1Print(Sender: TObject);
begin
   if DM1.cdsCobraCxC.FieldByName('TMONID').Value=DM1.wTMonLoc then
...
procedure TFCancelLiq.z2bbtnNuevoClick(Sender: TObject);
begin
   if DM1.cdsCobraCxC.FieldByName('NCOESTADO').Value ='I' then
   begin
      if wmodifica then
...
procedure TFCancelLiq.z2bbtnAnulaClick(Sender: TObject);
begin
    if DM1.cdsCobraCxC.FieldByName('NCOESTADO').Value='I' then
    begin
...
procedure TFCancelLiq.ActPnlBotones;
begin
...
FIN DE LA FORMA
*******************************************************}
procedure TFCancelLiq.FormClose(Sender: TObject; var Action: TCloseAction);
var sSQLAnt: string;
begin
...
Function TFCancelLiq.ExisteMovCxC(xxCia,xxTDiario,xxAAMM,xxNoReg:String):Boolean;
begin
  Result:=DM1.cdsMovCxC.Locate('CIAID;TDIARID;CCANOMES;CCNOREG',
                                VarArrayOf([xxCia,xxTDiario,xxAAMM,xxNoReg]),[]);
end;

procedure TFCancelLiq.edtClieRUCChange(Sender: TObject);
begin
   wmodifica:=True;
end;

procedure TFCancelLiq.dbeGiradoAChange(Sender: TObject);
begin
   wmodifica:=True;
end;

procedure TFCancelLiq.dbdtpFCobraChange(Sender: TObject);
begin
{   wmodifica:=True;}
end;

procedure TFCancelLiq.dbeNoChqChange(Sender: TObject);
begin
   wmodifica:=True;
end;

procedure TFCancelLiq.dbeElaboChange(Sender: TObject);
begin
   wmodifica:=True;
end;

procedure TFCancelLiq.dbeAprobChange(Sender: TObject);
begin
   wmodifica:=True;
end;

procedure TFCancelLiq.FormKeyPress(Sender: TObject; var Key: Char);
var
   xStr : String ;
begin
...
procedure TFCancelLiq.z2bbtnOKCabClick(Sender: TObject);
...
{      sSQL:=' SELECT CIAID,LICOID,FPAGOID,FPAGODES,TMONIDPAGO,TGE103.TMONABR TMONABR, SUM(LICOMTOABOLOC) LICOMTOABOLOC, SUM(LICOMTOABOEXT) LICOMTOABOEXT FROM CXC320 '+
     ...
     sSQL:= ' SELECT CIAID,LICOID,FPAGOID,'+
      ...
      sSQL:= ' SELECT LICOTCAMBL,CCNODOC,DOCID,CIAID,LICOID,FPAGOID,CXC320.BANCOID,TGE105.BANCOABR,FECPAGO,LICONOCHQ,CLIEID,TMONIDPAGO,TGE103.TMONABR TMONABR,LICOMTOABOLOC,LICOMTOABOEXT FROM CXC320 '+
             ' LEFT JOIN TGE103 ON TGE103.TMONID=CXC320.TMONIDPAGO '+
             ' LEFT JOIN TGE105 ON TGE105.BANCOID=CXC320.BANCOID '+
             ' WHERE LICOID='+QuotedStr(dblcNota.text)+' AND CIAID='+QuotedStr(dblcCia.Text);
...
procedure TFCancelLiq.dblcTMonExit(Sender: TObject);
begin
...
procedure TFCancelLiq.ConfiguraAccesos;
begin
...
procedure TFCancelLiq.Libera_Admin;
begin
    Pon( A1 , A2 , pnlBotones ) ;
end;

procedure TFCancelLiq.Libera_Consul;
begin

end;

procedure TFCancelLiq.Proc_Admin;
var
    Contador : Integer ;
begin
    pnlCabecera.Enabled := False ;
...
procedure TFCancelLiq.Proc_Consul;
begin
    pnlCabecera.Enabled := False ;
    pnlCabecera2.Enabled := False ;
    pnlDetalle.Enabled := True ;
...
procedure TFCancelLiq.LibConfigAccesos;
begin
    if DM1.wAdmin = 'G' then
       Libera_Admin
    else
...
procedure TFCancelLiq.Z2bbtnCancelarClick(Sender: TObject);
var
   xCobLoc ,
   xCobExt : Currency         ;
begin
...
//          DM1.cdsDetFPago.FieldByName('CCNODOC').AsString:=DM1.cdsQry3.FieldByName('CCNODOC').AsString;
...
//          DM1.cdsDetFPago.FieldByName('FC_FCANC').AsString:='26/10/2001';
        ...
        DM1.ActSdoCaja(DM1.cdsCobraCXC.FieldByName('TMONID').AsString, dblcBanco.text,DM1.cdsCobraCXC.FieldByName('NCOMTOORI').AsString,DateToStr(Date)) ;
        DM1.cdsQry3.Next;
        end;
        DM1.cdsQry3.MasterSource:=DM1.dsQry2;
        ...
        dm1.AplicaDatos(dm1.cdsCobraCXC,'COBRACXC');
        DM1.cdsCLiqCob1.ApplyUpdates(0);
...
procedure TFCancelLiq.GrabaEstadoFPago(xEstado: string);
begin
...
procedure TFCancelLiq.ValidaCancelacion;
begin
...
procedure TFCancelLiq.ActualizaPagados;
begin
   DM1.cdsCanjeCxC.DisableControls;
   DM1.cdsCanjeCxC.First;
   While not DM1.cdsCanjeCxC.Eof do
   Begin
      ...
      begin
         DM1.cdsMovCxC.edit;
         DM1.cdsMovCxC.fieldbyname('CCPAGLOC').AsCurrency :=
                       DM1.cdsMovCxC.fieldbyname('CCMTOLOC').AsCurrency -
                       DM1.cdsMovCxC.fieldbyname('CCSALLOC').AsCurrency  ;
         DM1.cdsMovCxC.fieldbyname('CCPAGEXT').AsCurrency :=
                       DM1.cdsMovCxC.fieldbyname('CCMTOEXT').AsCurrency -
                       DM1.cdsMovCxC.fieldbyname('CCSALEXT').AsCurrency  ;

         If DM1.cdsMovCxC.fieldbyname('TMONID').Value=DM1.wTMonLoc then
         begin
            DM1.cdsMovCxC.fieldbyname('CCPAGORI').AsCurrency :=
                                      DM1.cdsMovCxC.fieldbyname('CCPAGLOC').AsCurrency ;
            If DM1.cdsMovCxC.fieldbyname('CCSALLOC').AsCurrency <= 0.01 then
         ...
         begin
            DM1.cdsMovCxC.fieldbyname('CCPAGORI').AsCurrency :=
                                      DM1.cdsMovCxC.fieldbyname('CCPAGEXT').AsCurrency ;
            If DM1.cdsMovCxC.fieldbyname('CCSALEXT').AsCurrency <= 0.01 then
               DM1.cdsMovCxC.fieldbyname('CCESTADO').AsString := 'C';
         end;
         DM1.cdsMovCxC.fieldbyname('CCTCAMPA').AsCurrency := DM1.cdsCanjeCxCDoc.fieldbyname('CCTCAMCJE').AsCurrency ;
         DM1.cdsMovCxC.post ;
      end
      else
      begin
...
procedure TFCancelLiq.FormCreate(Sender: TObject);
var
 xsql : string;
begin
...
procedure TFCancelLiq.dbeTCambioEnter(Sender: TObject);
begin
{    strTmp := TCustomEdit(Sender).Text ;}
end;

procedure TFCancelLiq.dblcBancoExit(Sender: TObject);
var sWhere: string;
begin
   IF z2bbtnCancel.focused then exit;

//   if DiaAperturado(Date,dblcBanco.text) then
   begin
...
procedure TFCancelLiq.Validacab;
begin
...
procedure TFCancelLiq.dbdtpFCobraEnter(Sender: TObject);
begin
{      strTmp := dbdtpFCobra.Text ;}
end;

procedure TFCancelLiq.dbeElaboKeyPress(Sender: TObject; var Key: Char);
begin
   if key in ['0'..'9'] then
      key := #0;
end;

procedure TFCancelLiq.BorraGrid;
begin
...
procedure TFCancelLiq.FormDestroy(Sender: TObject);
begin
   cf1.Free ;
end;

procedure TFCancelLiq.dblcCiaExit(Sender: TObject);
...
procedure TFCancelLiq.dblcNotaEnter(Sender: TObject);
begin
...
procedure TFCancelLiq.Z2bbtnAdicFPagoClick(Sender: TObject);
begin
   pnlBotones.Enabled  := False;
...
procedure TFCancelLiq.CalculaResto;
var
...
begin
   ...
   begin
      ...
      dbeResta.Text:=CurrToStr( xMtoLoc - xMtoLoP +
                                dm1.cdsdetfpago.fieldbyname('FC_RECIBIDO').AsCurrency );
   end
   else
...
procedure TFCancelLiq.BitBtn2Click(Sender: TObject);
begin
...
procedure TFCancelLiq.Z2OkDetalleClick(Sender: TObject);
var
   xNuevo : Boolean;
begin
   ValidacionDatos;
...
procedure TFCancelLiq.ValidacionDatos;
begin
...
procedure TFCancelLiq.ValidacionTjaChq;
begin
...
procedure TFCancelLiq.ValidacionCobradoTotal;
begin
   if chkAsignaResto.Checked then
      Exit;
end;

procedure TFCancelLiq.GrabaDetalle;
var
    xMonLoc ,
    xMtoExt : Currency;
begin
...
//        FieldByName('CPTOID').AsString         := trim(dblcdCnp.Text);
        ...
        if dblctmonc.Text = dm1.wTMonLoc then
        begin
           xMonLoc := FieldByName('FC_COBRADO').AsCurrency;
           xMtoExt := Redondea( FieldByName('FC_COBRADO').AsCurrency / FieldByName('TC_USADO').AsCurrency );
        end
        else
        begin
           xMonLoc := FieldByName('FC_COBRADO').AsCurrency * FieldByName('TC_USADO').AsCurrency;
           xMtoExt := FieldByName('FC_COBRADO').AsCurrency;
        end;
        FieldByName('FC_MTOLOC').AsCurrency  := Redondea( xMonLoc );
        FieldByName('FC_MTOEXT').AsCurrency  := Redondea( xMtoExt );
        //fin de calcular
        Post;
    end;
end;

procedure TFCancelLiq.AcumulaDetFPago;
var
...
procedure TFCancelLiq.BitBtn1Click(Sender: TObject);
begin
   dm1.cdsDetFPago.Cancel;
   pnlFPago.SetFocus;
   pnlFPago.Visible := False;
...
procedure TFCancelLiq.dblcFormPagoExit(Sender: TObject);
begin
...
procedure TFCancelLiq.dblcTMonCExit(Sender: TObject);
begin
...
procedure TFCancelLiq.dbeRecibidoChange(Sender: TObject);
begin
    dbeACobrar.Enabled := True;
end;

procedure TFCancelLiq.dbeRecibidoEnter(Sender: TObject);
begin
   strTmp := dbeRecibido.Text;
end;

procedure TFCancelLiq.dbeRecibidoExit(Sender: TObject);
var
   Tmp : String;
begin
   Tmp := StrTmp;
...
procedure TFCancelLiq.dbeACobrarEnter(Sender: TObject);
begin
   strTmp := dbeACobrar.Text;
end;

procedure TFCancelLiq.dbeACobrarExit(Sender: TObject);
begin
...
procedure TFCancelLiq.dblcBancoFPagoExit(Sender: TObject);
begin
...
procedure TFCancelLiq.dblcTarjetaExit(Sender: TObject);
begin
...
procedure TFCancelLiq.chkAsignaRestoClick(Sender: TObject);
begin
  if chkAsignaResto.Checked then
  begin
...
procedure TFCancelLiq.DesabilitaClient;
begin
...
procedure TFCancelLiq.dbgDocCanjeRowChanged(Sender: TObject);
begin
//    z2bbtnSumatClick(Sender);
end;
