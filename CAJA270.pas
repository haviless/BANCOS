...
begin
   ...
   begin
	...
	Try
      ...
      DMTE.cdsProvCta.FieldByname('CIAID').AsString:=cCia;
...
begin
   ...
   begin
   cSQL:='select A.*, B.*, C.AUXDNI, case when AUXOBS=C.AUXGIRA THEN '' '' ELSE C.AUXGIRA END AUXGIRA, '
        +  'D.BANCOID BANCC, D.CCBCOID, E.BANCONOM, '
        +  'CASE WHEN A.CPTOID=''EG0173'' THEN ''REPRESENTANTES'' ELSE ''MOVILIDADES'' END TITULO '
        +'From CAJ_ORD_PAG_CAB A, CAJ_ORD_PAG_DET B, CNT201 C, CAJA102 D, TGE105 E '
        +'where A.CIAID=''' +DMTE.cdsPagoCxP.FieldByname('CIAID').AsString +''' '
        +  'AND A.NUMERO='''+DMTE.cdsPagoCxP.FieldByname('NUMERO').AsString+''' '
        + ' AND A.CIAID=B.CIAID AND A.NUMERO=B.NUMERO '
        + ' AND B.CLAUXID=C.CLAUXID(+) AND B.AUXID=C.AUXID(+) '
        + ' AND B.CLAUXID=D.CLAUXID(+) AND B.AUXID=D.PROV(+) '
        + ' AND D.BANCOID=E.BANCOID(+) '
        +'Order by ITEM';
   end;
...
procedure TFOrdenPago.sbDisenoRepClick(Sender: TObject);
begin
// Inicio HPC_201403_CAJA
