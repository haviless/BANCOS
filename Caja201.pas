...
// Actualizaciones      :
// HPC_201104_CAJA  19/09/2011  -  Boton para cambiar en Número de Cheque
// HPC_201302_CAJA  09/03/2013  -  Modificación de la contabilización del fondo fijo
//                                 para que genere asientos contables en la filial.
// HPC_201303_CAJA  19/03/2013  -  Modificación de la contabilización del fondo fijo
//                                 para que genere asientos contables en la filial en los
//                                 documentos provisionados.
// HPC_201304_CAJA  24/04/2013  -  Se modifica el valor de los argumentos al invocar
//                                 a la funcion "SOLConta" al momento de contabilizar
//                                 los asientos en la filial en la función "fg_ContabFilial"
// HPC_201401_CAJA  03/03/2014  -  Reordenamiento de la sangría a todo el formulario
//                                 Asignación de Control Transaccional, retiro de rutina APLICADATOS y CONTROLTRAN
//                                 Grabación a través de SQL directo a la Base de Datos
// HPC_201401_CAJA  27/02/2014  -  Movimientos de Caja Chica
...
Type
   TFEgresoCaja = Class(TForm)
    ...
    Label31: TLabel;
    lblMontoBase: TLabel;
    lblSaldoActual: TLabel;
    bbtnAlertaFdoFijo: TBitBtn;
   ...
   // Inicio HPC_201401_CAJA
      ...
      procedure bbtnAlertaFdoFijoClick(Sender: TObject);
   // Fin HPC_201401_CAJA
   ...
   Public
    { Public declarations }
      ...
      // Inicio HPC_201401_CAJA  27/02/2014  -  Movimientos de Caja Chica
      sCuentaCajaChica:String;
      sMonBase:Double;
      sMonSaldo:Double;
      // Fin HPC_201401_CAJA  27/02/2014  -  Movimientos de Caja Chica
...
Begin
   ...
   Begin
      xSQL := 'select A.CPTOIS, A.PROV, A.CPTOID, A.CPTODES, A.CUENTAID, FCTZ From CAJA201 A '
            + ' WHERE A.CPTOIS=''E'' '
...
// Inicio HPC_201401_CAJA -- Reposición de Caja Chica
   sCajaChica : String;
// Fin  HPC_201401_CAJA -- Reposición de Caja Chica
Begin
   ValidacionCabecera1;
...
// Inicio HPC_201401_CAJA -- Reposición de Caja Chica
   ...
   begin
       sSQL:='Select A.CUENTAID, B.BANCOID from CAJA201 A, TGE105 B '
            +' where A.CPTOID='''+dblcdCnp.Text+''' AND A.CUENTAID=B.CUENTAID(+)';
       DMTE.cdsQry.Close;
       ...
       begin
          If Not DiaAperturado(dblcCia.Text, dbdtpFComp.Date, sCajaChica) Then
             Exit;
       end;
   end;
// Fin  HPC_201401_CAJA -- Reposición de Caja Chica
...
// flag de detraccion
   ...
   // Inicio HPC_201401_CAJA
      ...
      if dbdtpFEmis2.Date >= strtodate('01/03/2014') then
      begin
         xRetLoc := DMTE.FRound(xRetLoc * 0.03, 15, 2);
         xRetExt := DMTE.FRound(xRetExt * 0.03, 15, 2);
      end
      else
      begin
         xRetLoc := DMTE.FRound(xRetLoc * 0.06, 15, 2);
         xRetExt := DMTE.FRound(xRetExt * 0.06, 15, 2);
      end;
   // Fin HPC_201401_CAJA
   End
   Else
   ...
   Begin
      ...
      // cambia forma de grabación
         ...
         // inserta en base a detalle del clientdataset
            ...
            While Not DMTE.cdsRegCxP.Eof Do
            Begin
               xSQL := 'Insert into CAJA301(CIAID, TDIARID, ECANOMM, ECNOCOMP, DOCID2, DOCMOD, CPTOID, TDIARID2, '
...
Var
   ...
   xSuma, cSalLoc, cSalExt, cSalOri: Double;
   cNodoc: String;
...
Begin
      ...
      Begin
         ...
         // Fin HPC_201401_CAJA
            ...
            Begin
                ...
                Begin
                   screen.Cursor := crDefault;
                   ShowMessage('Error al intentar actualizar Saldos de Caja');
                   DMTE.DCOM1.AppServer.RetornaTransaccion;
                   DMTE.cdsRegCxP.EnableControls;
                   Exit;
                End;
   ...
   // Inicio HPC_201401_CAJA
      ...
      ShowMessage('No se puede Cancelar porque el Estado es diferente de INICIAL');
   // Fin HPC_201401_CAJA
   End;
End;
...
Begin
   ...
   //Inicio HPC_201701_CAJA Actualización de série
...
// Inicio HPC_201401_CAJA
// retira código innecesario en ruta inexistente
...
Begin
   // Inicio HPC_201401_CAJA
   // Llama nuevamente a los CDS de CAJA302 Y CAJA301 PARA CONTABILIZAR
   xSql:= 'Select * From CAJA302 '
         +' Where CIAID='+ quotedstr(dblcCia.Text)
         +'   and ECANOMM='+ quotedstr(edtPeriodo.Text)
         +'   and TDIARID='+ quotedstr(dblcTDiario.Text)
   ...
   If (DMTE.cdsEgrCaja.RecordCount<=0) and
      (DMTE.cdsEgrCaja.FieldByName('ECESTADO').AsString<>'C') Then
   Begin
     ShowMessage('La Cabecera del comprobante no se grabo correctamente (CAJA302)');
     Exit;
   End;
   ...
   If (DMTE.cdsRegCxP.RecordCount<=0)  Then
   Begin
     ShowMessage('El Detalle del comprobante no se grabo correctamente (CAJA302)');
     Exit;
   End;
   ...
   Begin
      ...
      End;
         ...
         xSQL := 'select NVL(SUM(1),0) NUMERO from CNT300 where CIAID=''20'' AND TDIARID=''07'' AND CNTANOMM=''201404'' AND CNTCOMPROB=''0000000005'' ';
         DMTE.cdsQry.Close;
         DMTE.cdsQry.DataRequest(xSQL);
         DMTE.cdsQry.open;
         xNum300 := DMTE.cdsQry.FieldByName('NUMERO').AsInteger;
...
Var
   xSQL, sCajaChica, ssql : String; // HPC_201401_CAJA
Begin
   dbeRepo.text := '';
...
// Fin HPC_201401_CAJA
   ...
   // Fin  HPC_201401_CAJA
      ...
      // Inicio HPC_201401_CAJA Actualiza Saldos
         If wBcoTipCta = 'C' Then
         Begin
            DMTE.ActSaldosCaja(DMTE.cdsEgrCaja.FieldByName('CIAID').AsString, DMTE.cdsEgrCaja.FieldByName('TMONID').AsString,
               DMTE.cdsEgrCaja.FieldByName('BANCOID').AsString,
               DMTE.cdsEgrCaja.FieldByName('ECMTOORI').AsString, 'AE', DMTE.cdsEgrCaja.FieldByName('ECFCOMP').AsString);
         End
         Else
         Begin
            DMTE.ActSaldosBancos(DMTE.cdsEgrCaja.FieldByName('CIAID').AsString, DMTE.cdsEgrCaja.FieldByName('TMONID').AsString,
                                 DMTE.cdsEgrCaja.FieldByName('BANCOID').AsString,
                                 DMTE.cdsEgrCaja.FieldByName('CCBCOID').AsString,
            ...
            sCajaChica:=DMTE.cdsQry.FieldByName('BANCOID').AsString;
          // Inicio HPC_201401_CAJA
          // Actualiza el Saldo simpre que el cheque este pagado
          //if DMTE.cdsQry.FieldByName('CUENTAID').AsString=sCuentaCajaChica then
            if (DMTE.cdsQry.FieldByName('CUENTAID').AsString=sCuentaCajaChica) AND
               (Length(Trim(DMTE.cdsEgrCaja.FieldByName('ECFPAGOP').AsString))>0) Then
          // Final HPC_201401_CAJA
            begin
               DMTE.ActSaldosCaja(DMTE.cdsEgrCaja.FieldByName('CIAID').AsString,
                                  DMTE.cdsEgrCaja.FieldByName('TMONID').AsString,
                                  sCajaChica,
                                  DMTE.cdsEgrCaja.FieldByName('ECMTOORI').AsString,
                                  'AI',
                                  DMTE.cdsEgrCaja.FieldByName('ECFCOMP').AsString);
         ...
         End;
      // Fin  HPC_201401_CAJA Actualiza Saldos
      End;
   ...
   //   DMTE.ControlTran(0, DMTE.cdsEgrCaja, 'EGRCAJA');
      ...
      Try
         DMTE.DCOM1.AppServer.EjecutaSQL(xSQL);
      Except
         ShowMessage('Error al intentar actualizar el flag de ANULADO en la cabecera del Egreso');
         DMTE.DCOM1.AppServer.RetornaTransaccion;
         Exit;
      End;
...
Begin
   ...
   xSQL:='Select ECESTADO From  CAJA302 '
         +'Where CIAID='+quotedstr(dblcCia.Text)
         +'   and ECANOMM='+quotedstr(edtPeriodo.Text)
         +'   and TDIARID='+quotedstr(dblcTDiario.Text)
         +'   and ECNOCOMP='+quotedstr(dbeNoComp.Text);
   ...
   DMTE.cdsQry1.Open;
   If (DMTE.cdsQry1.RecordCount>0)  and (DMTE.cdsQry1.FieldByName('ECESTADO').AsString = 'X') Then
...
Begin
   ...
   Begin
      ...
      // Fin HPC_201401_CAJA
        ...
        //Alerta de FONDO FIJO
        If (dblcCia.Text='02')  and
           (dblcBanco.Text='20') Then
        Begin
          pnlCabecera1.Enabled := false;
          DMTE.cdsEgrCaja.Edit;
          DMTE.cdsEgrCaja.FieldByName('DOCID').AsString :='74';
          DMTE.cdsEgrCaja.FieldByName('TMONID').AsString :='N';
          DMTE.cdsEgrCaja.FieldByName('CPTOID').AsString :='EG0141';
          dblcTMon.Color := clInfoBk;
          dblcTDoc.Color := clInfoBk;
          dblcdCnp.Color := clInfoBk;
        ...
        Begin
         ...
         dblcTMon.Color := clWindow;
         dblcTDoc.Color := clWindow;
         dblcdCnp.Color := clWindow;
...
Procedure TFEgresoCaja.ProveedoresNotificados;
Var
   xWhere, sRes: String;
Begin
...
Begin
   ...
   Else
   Begin
     //para las movilidades se activa el ingreso del auxiliar
     If(DMTE.cdsCptos.FieldByName('FCTZ').AsString   ='M')    and
       (DMTE.cdsCptos.FieldByName('CPTOIS').AsString ='E')    and
       (DMTE.cdsCptos.FieldByName('PROV').AsString   ='CAJCH')Then
     Begin
       DMTE.cdsRegCxP.FieldByName('CLAUXID').AsString := 'E';
       dblcClaseDet.Text:='E';
       dblcClaseDet.Enabled := True;
       dblcdAuxDet.Enabled  := True;
       dblcClaseDetExit(SELF);
     End
     Else
     Begin
       dblcdAuxDet.Enabled := False;
...
Begin
// Inicio HPC_201401_CAJA
// Se pone en comentarios, no se utiliza en el módulo
   {
   cSQL := 'UPDATE CXP301 '
          +'   SET CPPAGORI =' + '' + floattostr(ccPgoOri) + '' + ', '
   ...
   If dblcTMon.text = DMTE.wTMonExt Then
      If (StrToFloat(xsaldo) <= 0.02) Then
         cSQL := cSQL + ' ,CPESTADO = ''C'''
      Else
         If (StrToFloat(xsaldo) <= 0.02) Then cSQL := cSQL + ' ,CPESTADO = ''C''';

   cSQL := cSQL + 'WHERE CIAID   =    ' + '''' + trim(dblcCia.Text) + '''' + ' AND  '
                + '      DOCID   =    ' + '''' + wTDocRxH + '''' + ' AND  '
                + '      CPNODOC = ' + '''' + ccNoDoc + '''' + ' AND  '
                + '      CPESTADO <> ' + '''' + 'E' + '''' + ' AND  '
...
Procedure TFEgresoCaja.bbtnCHClick(Sender: TObject);
Begin
...
Procedure TFEgresoCaja.bbtnGraClick(Sender: TObject);
Begin
...
Procedure TFEgresoCaja.bbtnCanCHClick(Sender: TObject);
Begin
   pnlCH.Visible := False;
End;

Procedure TFEgresoCaja.fg_ObtieneCtaporCentroCosto(wgCentroCosto: String);
Var
   xsSql: String;
Begin
...
Procedure TFEgresoCaja.fg_ContabFilial(wgCia, wgPeriodo, wgDiario, wgNumComprob: String);
Var
   xsSql: String;
...
// Fin HPC_201401_CAJA
   ...
   Begin
      ...
      If not xSigueGrab then exit; // HPC_201401_CAJA
...
Procedure TFEgresoCaja.fg_ContabInsPago(wgCia, wgPeriodo, wgDiario, wgNumComprob: String; wgRegistro: Integer);
Var
   xsSql: String;
...
Begin
   ...
   Except
      ...
      xSigueGrab := False; // HPC_201401_CAJA
      Exit;
   End;
End;

Procedure TFEgresoCaja.fg_InsertaFilialCaja302(wgCiaPrincipal, wgDiario,
...
Function TFEgresoCaja.fg_ObtieneNumComprob(wgCompania, wgPeriodo,
   wgDiario: String): String;
Var
   xsSql: String;
Begin
...
Procedure TFEgresoCaja.fg_EliminarContaFilial(wgCiaPrinc, wgDiarioPrinc, wgPeriodoPrinc, wgNumPrinc: String);
Var
   xsSql: String;
Begin
   xsSql := ' DELETE CNT301 '
...
Function TFEgresoCaja.fg_ExisteContabFilial(wgCiaPrincipal, wgPeriodo, wgTiario, wgNumero: String): Boolean;
Var
   xsSql: String;
Begin
   xsSql := 'SELECT * '
...
Function TFEgresoCaja.fg_BuscaDocProvisionado(wgTipoAuxiliar, wgProveedor, wgTipoDocumento, wgSerie, wgNumDocumento: String): Boolean;
Var
   xsSql: String;
   xsCiaDestino: String;
Begin
...
Procedure TFEgresoCaja.dbeNoDoc2Exit(Sender: TObject);
Begin
...
// Inicio HPC_201401_CAJA
procedure TFEgresoCaja.sbDisenoRepClick(Sender: TObject);
begin
...
// Inicio HPC_201401_CAJA
procedure TFEgresoCaja.AlertaFondo;
var xsql:String;
begin
     //aqui recupero el monto base del fondo fijo y el ultimo saldo
       xSQL :='Select MONBASE From TGE106 where Bancoid='''+dblcBanco.Text+''' AND CIAID='''+dblcCia.Text+''' ';
       DMTE.cdsQry1.Close;
       DMTE.cdsQry1.DataRequest(xSql);
       DMTE.cdsQry1.Open;
       ...
       Else sMonBase:= 0.00;
        ...
        xSql := 'SELECT A.STSALDOFIN  FROM  '
               +'(SELECT STSALDOFIN FROM CAJA310 A WHERE CIAID='''+dblcCia.Text+''' AND BANCOID='''+dblcBanco.Text+''' AND STFECHA<=TRUNC(SYSDATE) ORDER BY STFECHA DESC ) A  '
               +' WHERE ROWNUM=1 ';
        DMTE.cdsQry1.Close;
        DMTE.cdsQry1.DataRequest(xSql);
        DMTE.cdsQry1.Open;
        ...
        Begin
           ...
           pnlAlerta.Left := 281;
           pnlAlerta.Top  := 61;
           lblSaldoActual.Caption := FormatFloat('###,###.00', sMonSaldo);
           lblMontoBase.Caption   := FormatFloat('###,###.00', sMonBase);
        End
        Else
        Begin
         // Estado de componentes
...
// Inicio HPC_201401_CAJA
procedure TFEgresoCaja.bbtnAlertaFdoFijoClick(Sender: TObject);
begin
